...
__global__ void gpu_brute_force(
  ...
  for (int i=0; i<sp_sizes[sp_sizes_size - 1]; i++) {
    ...
    if (pattern[p_size - 1 - i] == '[') {
      if ((bitmasks[last_bm_idx*4 + data[offset + (int64_t)sizes[tid] - 1 - i]/64] & 
          ((int64_t)1 << (data[offset + (int64_t)sizes[tid] - 1 - i]%64))) == 0) return;
      last_bm_idx--;
      continue;
    }
    if (data[offset + (int64_t)sizes[tid] - 1 - i] != pattern[p_size - 1 - i]) return;
  }
...
__global__ void gpu_brute_force_rid(
    char* data, 
    int64_t* offsets,
    int* sizes, 
    size_t table_size, 
  ...
  result_rids[atomicAdd(result, 1)] = tid;
}