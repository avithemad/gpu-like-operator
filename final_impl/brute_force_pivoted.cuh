...
__global__ void gpu_brute_force_pivoted(
    char* data, 
    int* sizes,
    size_t table_size, 
    int* result, 
  ...
  for (int64_t i=0; i<sp_sizes[0]; i++) {
    ...
    if (pattern[i] == '[') {
      if ((bitmasks[bm_idx*4 + data[i*table_size + tid]/64] & ((int64_t)1 << (data[i*table_size + tid]%64))) == 0) return;
      bm_idx++;
      continue;
    }
    if (data[i*table_size + tid] != pattern[i]) return;
  }
  ...
  for (int i=0; i<sp_sizes[sp_sizes_size - 1]; i++) {
    ...
    if (pattern[p_size - 1 - i] == '[') {
      if ((bitmasks[last_bm_idx*4 + data[tid + ((int64_t)sizes[tid] - 1 - i) * table_size]/64] & 
          ((int64_t)1 << (data[tid + ((int64_t)sizes[tid] - 1 - i) * table_size]%64))) == 0) return;
      last_bm_idx--;
      continue;
    }
    if (data[tid + ((int64_t)sizes[tid] - 1 - i) * table_size] != pattern[p_size - 1 - i]) return;
  }
  ...
  for (int64_t i=1; i < sp_sizes_size-1; i++) { // for each subpattern, conduct substring matching
    bool sp_match = false;
    int bmi;
    for (int64_t j = s_start; j < s_end - sp_sizes[i] + 1; j++) {
      bool matched = true;
      bmi = bm_idx;
      for (int64_t k = 0; k < sp_sizes[i]; k++) {
        if (pattern[p_offset + k] == '_') continue;
        if (pattern[p_offset + k] == '[') {
          if ((bitmasks[bmi*4 + data[tid + (j + k) * table_size]/64] & 
              ((int64_t)1 << (data[tid + (j + k) * table_size]%64))) == 0) {
            matched = false;
            break;
          }
          bmi++;
        ...
        if (data[tid + (j + k) * table_size] != pattern[p_offset + k]) {
          matched = false;
          break;
        }
      }
...
__global__ void gpu_brute_force_pivoted_step(
    char* data, 
    int* sizes,
    size_t table_size, 
    int* result, 
